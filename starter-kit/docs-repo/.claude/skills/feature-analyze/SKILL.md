...
description: "Read-only cross-cutting consistency and coverage audit of a whole feature across the repo seam. Emits severity-ranked findings, each routed to the stage that owns the repair, then STOPS."
argument-hint: "<NNN-slug>"
compatibility: "Requires the multi-root workspace — <product>-docs/ and the code repos as siblings"
metadata:
  author: "<product>-docs"
  source: ".github/agents/feature.analyze.agent.md"
user-invocable: true
disable-model-invocation: true
---

# Stage 5a — Analyze (Tech Lead)
...
2. **`<workspace>/<product>-docs/.github/agents/feature.analyze.agent.md`** — **that file is your
...
Dispatches read-only `repo-auditor` sub-agents, one per dimension, all in ONE message. READ-ONLY: despite carrying editFiles, this stage may append only one analyze log line plus Open Questions. A finding is routed to its owning stage — never repaired here.
