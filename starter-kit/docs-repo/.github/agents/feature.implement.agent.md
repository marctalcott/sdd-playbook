...
- **Implement against `TBD` slices.** If the story's slices are still `TBD`, STOP: "run
  ...
  sequencing.
- **Invent API contracts or domain names.** Consume the merged/planned contracts and `glossary.md`
...
- Read its `spec.md` / `plan.md` / `data-model.md` / `contracts/`, its constitution
  (`manifest.repos.<repo>.constitution`), and `glossary.md` — and **build strictly within the
