...
- **Inventing domain terms, personas, decisions, or acceptance criteria.** Everything in
  ...
  (`glossary.md`, `personas-and-jobs.md`, `decisions.md`). A missing term is an **Open
...
1. From the input, determine the candidate `NNN`/slug. For a catalog ID, open
   `<workspace>/<product>-docs/feature-catalog.md`, find the row, and note its short name and
...
2. Mark a story **excluded** when: a supersession marker from step 3 flags it; a decision in
   `decisions.md` supersedes it; it is backend-only with no UI surface (when the remaining
...
2. Write **`feature.md`** — the ONE cross-repo spec, **synthesized from the existing specs**.
   ...
   - **Governing principles** (cite `P-XX`) and **decisions** (cite `D-XXX`), including any that
     drive an exclusion.
   - **Canonical names** pulled from `glossary.md`. Nothing coined.
...
- **Names are law.** A `Title` means the same thing in the docs, the API, and the UI. If
  `feature.md` needs a term that isn't in `glossary.md`, flag it — that's a signal to add it
